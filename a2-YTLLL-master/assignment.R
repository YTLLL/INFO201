...
movies <- c("The Dark Knight", "The Godfather", "Inception", "Roman Holiday", "The Matrix", "Leon")
...
make_birthday_intro <- function(name, age, birthday) {
  days <- as.Date(birthday) - today
  intro1 <- paste("In ", days, " days I'll be ", age + 1, ".", sep = "")
  return(cat(paste(make_introduction(name, age), intro1, sep = "\n")))
}
...
my_bday_intro <- make_birthday_intro(my_name, my_age, "2019-11-29")

# Challenge ------------------------------------------------------------------
# Write a function `remove_digits` that will remove all digits
# (i.e., 0 through 9) from all elements in a *vector of strings*.
remove_digits <- function(courses) {
  new <- gsub("\\d", "", courses)
}

# Demonstrate that your approach is successful by passing a vector of courses
# to your function. For example, remove_digits(c("INFO 201", "CSE 142"))
new_course <- remove_digits(c("INFO 201", "CSE 142"))
